...
def find_run_folder(path: str):
    """Finds the folder in the given path that starts with 'run'."""
...
def main(cfg: DictConfig) -> None:
    parent_folder = cfg.runs_parent_folder
    config_location_in_run_folder= cfg.config_location_in_run_folder
    checkpoint_dirs_location_in_run_folder= cfg.checkpoint_dirs_location_in_run_folder


    run_folders = [f.path for f in os.scandir(parent_folder) if f.is_dir()]
    configs = [f + config_location_in_run_folder for f in run_folders]
    checkpoint_dirs = [f + checkpoint_dirs_location_in_run_folder for f in run_folders]

    assert len(checkpoint_dirs) == len(configs), "Number of checkpoint dirs and configs should be the same."
    for config, checkpoint_dir in zip(configs, checkpoint_dirs):
        assert(find_run_folder(config) == find_run_folder(checkpoint_dir)), "Both paths should contain the same run folder."
    ...
    for config, checkpoint_dir in zip(configs, checkpoint_dirs):
        ...
        # Check if both paths contain a folder that starts with "run-"
        if not any(path.parts[-2].startswith("run-") for path in [config_file_path, checkpoint_dir_path]):
            raise ValueError("Both paths should contain a folder that starts with 'run-'.")
